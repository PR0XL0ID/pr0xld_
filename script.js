...
document.addEventListener('keydown', event => {
    if (event.ctrlKey && (event.key === 'c' || event.key === 'u' || event.key === 'v' || event.key === 'i' || event.key === 'x' || event.key === 'F12')) {
...
function openModal(gameUrl, gameTitle, isRummerhead = false) {
    ...
    if (isRummerhead) {
        modalContent.classList.add('modal-rummerhead');
    } else {
        modalContent.classList.remove('modal-rummerhead');
    }
...
window.onclick = function (event) {
    ...
    if (event.target == gameModal) {
        closeModal();
    } else if (event.target == proxyModal) {
        closeProxyModal();
